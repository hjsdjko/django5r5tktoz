		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"auto","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('chengjixinxi','成绩情况')" type="success" @click="chartDialog1()">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var fenshuChart1 = echarts.init(document.getElementById("fenshuChart1"),'macarons');
        this.$http({
            url: "chengjixinxi/sectionStat/fenshu/kechengmingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    yAxis3.push(parseFloat((res[i].良好)));
	...
	chengjixinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'chengjixinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
